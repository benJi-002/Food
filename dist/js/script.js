...
window.addEventListener('DOMContentLoaded', () => {
  ...
  new MenuCard("img/tabs/post.jpg", "post", 'Меню "Постное"', 'Меню "Постное" - это тщательный подбор ингредиентов: полное отсутствие продуктов животного происхождения, молоко из миндаля, овса, кокоса или гречки, правильное количество белков за счет тофу и импортных вегетарианских стейков.', 14, '.menu .container', 'menu__item').render(); // Формы
  ...
  function postData(form) {
    form.addEventListener('submit', e => {
      ...
      statusMessage.style.cssText = `
                ...
                margin: 0 auto;
            `;
      form.insertAdjacentElement('afterend', statusMessage); // реализация POST при помощи XMLHttpRequest
      // const req = new XMLHttpRequest();
      // req.open('POST', 'server.php');
      // При связке XMLHttpRequest + объект + FormData заголовок устанавливать не нужно !!!
      // req.setRequestHeader('Content-type', 'application/json');

      const formData = new FormData(form); // реализация POST при помощи XMLHttpRequest (для этого метода это тоже нужно)
